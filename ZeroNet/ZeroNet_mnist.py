...
# best
# Adam,lr=0.009, epochs=50, batch_size=100, structure=(28*28,16), (16,16)x3, (16,10)
...
def train_mnist(model, epochs, train_dataloader, x_test, y_test, criterion):
    for epoch in range(epochs):
        tmp_loss = 0
        for data in train_dataloader():
            x_train, y_train = data
...
if __name__ == '__main__':
    ...
    # training
    start = time.time()
    train_mnist(model, epochs, train_dataloader, x_test, y_test, criterion)
    end = time.time()
    ...
    # # 画loss图
    # plt.plot(range(50), test_loss, label='test_loss', c='b')
    # plt.plot(range(50), train_loss, label='train_loss', c='r')
